...
from tools import AverageMeter, knn_monitor_cil
...
def main(device, args):
    ...
    for t in tqdm(range(0, args.n_tasks), desc='Evaluatinng'):
      ...
      #ckpt_dir = "/home/deepakprasad/OCL/UCL/checkpoints/Imagenet32-32_5T_alp_0.4_results/"
      #mixup_ucl_simsiam-tinyimg-experiment-resnet18_mixup_run_0
      #model_path  = f"{ckpt_dir}_{t}.pth"
      ...
      cl_default = True
      for i in eval_tids:
          acc = knn_monitor_cil(args,model.net.module.backbone, dataset, memory_data_loaders =  dataset.memory_loaders, test_data_loader= dataset.test_loaders[i],
                                device=device, cl_default = cl_default, task_id=i, k=min(args.train.knn_k, len(eval_tids)))
          #acc, acc_mask = = knn_monitor_cil( net = model, memory_data_loaders = memory_loaders, test_data_loader= test_loaders[i], task_id= i)
